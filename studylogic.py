import collections
import os
from utility import counterbalance, realign, find_diff, speed, writedata
...
def initblocks(cfg):
    ...
    for block in range(cfg['starting_block'], cfg['num_blocks']-1):
...
def runtrial(cfg, block, trial_number):
    ...
    while trial_data['step'] is not 'end':
        ...
        for stim in block['garage_stim'][trial_number]:
            stim.draw()
        cfg['win'].flip()
        trial_data['hand_pos'].append(cfg['mouse'].getPos())
