...
def writedata(cfg, block, trial_data):
    outfilename = 'Data/' + 'P' + str(cfg['P_number']).zfill(3) + '-' + str(block['number']) + '-' \
                  + str(trial_data['trial_number']) + '.txt'
    trial_number = trial_data['trial_number']
    with open(outfilename, 'wb') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter='\t', quoting=csv.QUOTE_MINIMAL)
        csv_writer.writerow(['time', 'trial', 'cursor_x', 'cursor_y', 'hand_x', 'hand_Y', 'rotation', 'step', 'target_angle',
                             'targetpos_x', 'targetpos_y', 'garagelocation', 'garagepos_x',
                             'garagepos_y', 'homepos_x', 'homepos_y','group', 'trial_type'])
...
def speed(pos,time, limit):
    current_time = time[-1]
    first_time = next(x[0] for x in enumerate(time) if x[1] >= current_time - 0.1)
